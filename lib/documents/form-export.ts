        ...
        pieces.forEach((piece, i) => {
          ...
          if (text) {
            runs.push({
              ...
              color,
              fontSize,
              align: isFirstInPara ? align : undefined,
              newline: i < pieces.length - 1
            })
            isFirstInPara = false
...
function drawRowBackgrounds(page: PDFPage, fields: PositionedField[]): void {
...
function drawTable(
  ...
  let curY =
    field.y + tableBlockHeight(cfg, field.tableLabelPosition === 'above') // top of table in PDF coords
  ...
  if (field.tableLabelPosition === 'above' && field.label) {
    ...
    const titleTxt = safeColor(
      cfg.titleTextColor ?? cfg.headerTextColor,
      '#ffffff'
    )
  ...
  const tableTop =
    field.y + tableBlockHeight(cfg, field.tableLabelPosition === 'above')
  page.drawRectangle({
    x: field.x,
    y: curY,
    width: field.width,
...
export async function exportFormPdf(
  ...
  const boldObliqueFont = await pdfDoc.embedFont(
  ...
  const presizedFields = rawFields.map(field => {
    ...
    const runs = parseHtmlRuns(
      field.contentHtml ?? field.content ?? field.label
    )
    type Tok = { text: string; bold?: boolean; italic?: boolean; nl?: boolean }
    const tokens: Tok[] = []
    for (const run of runs) {
      if (run.newline) {
        tokens.push({ text: '', nl: true })
        continue
      }
    ...
    for (const tok of tokens) {
      ...
      if (curLineW + ww > fieldWidth && curLineW > 0) {
        lineCount++
        curLineW = 0
      }
      curLineW += ww
    ...
    const computedH = Math.max(
      field.height || 18,
      padT + padB + lineCount * lineHeight
    )
    return { ...field, height: computedH }
  })

  const fields = applyAutoLayout(
    presizedFields,
    CONTENT_TOP - (titleStyle?.spacingBelow ?? 0)
  )
  ...
  for (const field of fields) {
    ...
    const labelFontPt = (defaultPt: number) => {
      ...
      if (process.env.NODE_ENV !== 'production') {
        console.log(
          `[label-font] field=${field.label} labelHtml=${field.labelHtml?.slice(0, 80)} → size=${result}`
        )
      }
      return result
    }
    ...
    if (field.type === 'text') {
      ...
      page.drawSvgPath(
        roundedRectPath(field.width, totalH, r, rTL, rTR, rBR, rBL),
        {
          x: field.x,
          y: field.y + totalH,
      ...
      if (rBL > 0 || rBR > 0) {
        page.drawSvgPath(
          roundedRectPath(field.width, field.height, 0, 0, 0, rBR, rBL),
          { x: field.x, y: field.y + field.height, color: rgb(1, 1, 1) }
        )
      } else {
        page.drawRectangle({
          x: field.x,
      ...
      page.drawText(labelText, {
        x: lx,
        y: field.y + field.height + (LABEL_SPACE - lSize) / 2,
        size: lSize,
        font: labelFont,
        color: rgb(labelC.r, labelC.g, labelC.b)
      })
      if (labelHasUnderline) {
        const ulY =
          field.y + field.height + (LABEL_SPACE - lSize) / 2 - lSize * 0.12
        page.drawLine({
          start: { x: lx, y: ulY },
          end: { x: lx + lw, y: ulY },
          thickness: Math.max(0.5, lSize * 0.07),
          color: rgb(labelC.r, labelC.g, labelC.b)
        })
      }
      ...
      if (bw > 0) {
        page.drawLine({
          start: { x: field.x, y: field.y + totalH },
          end: { x: field.x + field.width, y: field.y + totalH },
          thickness: bw,
          color: bcRgb
        })
        page.drawLine({
        ...
        page.drawLine({
          start: { x: field.x, y: field.y },
          end: { x: field.x, y: field.y + totalH },
          thickness: bw,
          color: bcRgb
        })
        page.drawLine({
          start: { x: field.x + field.width, y: field.y },
          end: { x: field.x + field.width, y: field.y + totalH },
          thickness: bw,
          color: bcRgb
        })
      ...
      page.drawLine({
        start: { x: field.x, y: field.y + field.height },
        end: { x: field.x + field.width, y: field.y + field.height },
        thickness: Math.max(bw, 0.5),
        color: bcRgb
      })
      ...
      textField.addToPage(page, {
        x: field.x + insL,
        y: field.y + insB,
        width: field.width - insL - insR,
        height: field.height - insB,
        borderWidth: 0,
      ...
      if (isSingle) {
        ...
        page.drawSvgPath(roundedRectPath(field.width, totalH, r, r, r, r, r), {
          x: field.x,
          y: field.y + totalH,
          color: rgb(labelBg.r, labelBg.g, labelBg.b)
        })
        ...
        const rowH = Math.max(
          field.height / Math.max(numRows, 1),
          radioSize + 4
        )
        ...
        for (let i = 0; i < allOpts.length; i++) {
          ...
          if (col === 0 && row > 0) {
            page.drawLine({
              start: { x: field.x, y: rowBottom + rowH },
              end: { x: field.x + field.width, y: rowBottom + rowH },
              thickness: 0.3,
      ...
      for (const run of runs) {
        ...
        if (run.newline) {
          tokens.push({ text: '', nl: true, align: curAlign })
          continue
        }
        for (const w of run.text.split(/\s+/).filter(Boolean)) {
          tokens.push({
      ...
      const tokenFont = (tok: Token) =>
        tok.bold ? boldFont : tok.italic ? italicFont : regularFont
      ...
      if (r > 0 && bw > 0) {
        ...
        if (btw > 0)
          page.drawLine({
            start: { x: field.x, y: field.y + staticH },
            end: { x: field.x + field.width, y: field.y + staticH },
            thickness: btw,
            color: bcRgb
          })
        if (bbw > 0)
        ...
        if (blw > 0)
          page.drawLine({
            start: { x: field.x, y: field.y },
            end: { x: field.x, y: field.y + staticH },
            thickness: blw,
            color: bcRgb
          })
        if (brw > 0)
